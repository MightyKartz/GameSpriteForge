...
func _initialize() -> void:
	...
	if asset_type == "map":
		...
		quit(0)
		return
	if asset_type in ["icon_set", "prop_set", "portrait_set", "equipment_set", "decal_set"]:
...
func _install_static_set(helper: Dictionary, target_res: String, asset_type: String) -> void:
	...
	var scenes_res := target_res.path_join("scenes")
	if asset_type in ["prop_set", "equipment_set", "decal_set"]:
		DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(scenes_res))
	for item_value in items:
		...
		var scene_item := asset_type == "prop_set" or asset_type == "decal_set"
		if asset_type == "equipment_set":
			scene_item = role == "world_prop" or role == "equipped_preview"
		if scene_item:
			var root := Node2D.new()
			root.name = "ForgeDecal" if asset_type == "decal_set" else "ForgeStaticAsset"
			var sprite := Sprite2D.new()
