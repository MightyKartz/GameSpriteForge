...
impl ComfyClient {
    ...
    pub fn doctor(&self, stored: &StoredProfile) -> Result<ComfyDoctor, ComfyError> {
        ...
        let mapped = &stored.profile.prompt_input;
        let prompt_class = workflow[&mapped.node]["class_type"].as_str().unwrap();
        let declared = &available.get(prompt_class).unwrap_or(&Value::Null)["input"];
        let mapped_valid = ["required", "optional", "hidden"]
            .iter()
            .any(|group| declared[*group].get(&mapped.input).is_some());
        let missing_inputs = if mapped_valid {
            vec![]
        } else {
            vec![format!("{}.{}", mapped.node, mapped.input)]
        };
