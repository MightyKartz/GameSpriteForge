...
| 引擎播放 | 既有测试已检查保存资源、采样率、声道、时长及循环范围 | 增加独立原生 stream mixer 检查：推进超过一次完整时长后音乐仍输出样本，音效已结束 |
...
6. 独立 Godot 脚本加载保存的 `.res`/道具场景，检查格式、循环范围、锚点和纹理
   采样，再通过 Godot 自身的 `AudioStreamPlayback.mix_audio` 推进 1.5 秒混音帧，
   检查音乐循环后的非零样本及一次性音效结束状态。
...
- **真实引擎：** 保存资源加载、引擎采样设置、原生混音的非零样本、音乐跨循环继续、音效结束。
  手动推进引擎混音不依赖 CI 的设备音频时钟，不提供扬声器/设备试听或无缝循环观感证据。
...
CI 曾在 macOS/Godot 4.6.3 的固定 1.5 秒等待后发现 one-shot 仍未结束，
而 Windows 与本机通过。此断言把场景树计时误作设备音频推进证据；保留失败产物，
改用[Godot 原生混音接口](https://docs.godotengine.org/en/4.6/classes/class_audiostreamplayback.html#class-audiostreamplayback-method-mix-audio)
显式推进流并检查返回样本与结束状态，不放宽为“仅能加载即通过”。最终 CI 必须在修正后的提交重新通过。
