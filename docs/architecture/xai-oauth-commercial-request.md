...
1. May a distributed third-party commercial CLI application use the shared public client ID `b1a00492-073a-47ea-816f-4c329264a828`, or can xAI issue a Forge-specific public client ID?
...
- No credential, device code, authorization header, or temporary media URL in jobs, packs, CLI JSON responses, or ordinary logs.
- Agents use CLI Provider listing/checking and generation planning; interactive authorization remains a user terminal operation.
...
Forge uses Device Code OAuth, stores credentials only in the user's operating-system credential store, never exposes credentials in agent-facing CLI output, and downloads Imagine output directly into a user-owned local job directory for deterministic processing. API Key authentication remains available.
